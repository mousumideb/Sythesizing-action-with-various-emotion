...
%
% Loads BVH file specified by FNAME (with or without .bvh extension) and
% parses the file, calculating joint kinematics and storing the output in
% SKELETON.
%
% Some details on the BVH file structure are given in "Motion Capture File
% Formats Explained":
% http://www.dcs.shef.ac.uk/intranet/research/resmes/CS0111.pdf But most of
...
%% Parse data
%
% This is a cheap tokeniser, not particularly clever. Iterate word-by-word,
% counting braces and extracting data.
...
while ~strcmp( C{ii} , 'MOTION' )
  ...
  elseif strcmp( token , 'CHANNELS' )
    ...
    % The 'order' field is an index corresponding to the order of 'X' 'Y'
    % 'Z'. Subtract 87 because char numbers "X" == 88, "Y" == 89, "Z" ==
    % 90.
...
%  - 5 lines per joint - 4 lines per chain end - 4 additional lines (first
%  one and last three)
...
%
% We have three possibilities for each node we come across: (a) a root node
% that has displacements already defined,
...
% six, three, and zero "channels" of data. In this section of the code, the
% channels are read in where appropriate and the relevant arrays are
% pre-initialised for the subsequent calcs.
...
% For an end effector we don't have rotation data; just need to calculate
% the final position.
...
%
% The third input, ORDER, is a vector indicating which order to apply the
% planar rotations. E.g., [3 1 2] refers applying rotations RXYZ around Z
% first, then X, then Y.
%
...
% matrix for, say, 'ZXY' which seems more common in BVH files. Should
% revisit this assumption one day.
%
